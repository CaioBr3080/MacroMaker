...
export const FIELD_HELP = {
  ...
  rotateTowardsTarget: "Aponta a animação não esticada da origem para o destino escolhido. Quando estica, o próprio Sequencer já mantém a direção do destino.",
  ...
  promptSaveMode: "Somente nesta execução usa a resposta apenas enquanto o macro está rodando. Permanentemente no macro grava o novo valor para as próximas execuções e requer que o usuário possa editar o macro.",
  promptCancelBehavior: "Cancelar macro interrompe a execução se a janela for fechada. Manter valor e continuar não muda a variável.",
  inputLabel: "Texto exibido acima do campo de resposta para orientar quem está executando o macro.",  operation: "Na etapa Definir variável: definir substitui o valor; somar, subtrair e multiplicar calculam a partir do atual. No Token Magic: aplicar adiciona, atualizar altera filtros existentes e remover exige o Filter ID.",
  ...
  preset: "Nome de um preset disponível no Token Magic FX. Use a lista como sugestão ou digite o nome de um preset próprio.",
  filters: "Lista JSON avançada de filtros Token Magic. Use-a quando não houver um preset; ela deve ser uma lista, por exemplo [{ filtro: valor }].",
  filterId: "Identificador do filtro Token Magic. É necessário para remover somente o efeito desejado sem afetar os demais.",
  replace: "Ao aplicar, remove os filtros Token Magic existentes do objeto antes de adicionar este efeito.",
  ...
  scale: "Tamanho base da animação: 1 = original, 0.5 = metade, 2 = dobro. O Sequencer ignora a escala quando a animação estica até o destino.",
  ...
  rotation: "Em graus. Sem apontar ao destino, gira a animação; com Apontar para o destino e sem esticar, funciona como ajuste fino da direção. Ex.: 90 gira um quarto de volta.",
